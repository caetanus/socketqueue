...
class BasicSocketQueueTestCase(unittest.TestCase):
    def test_auto_select(self):
        m = sockqueue.SocketQueue()
        if sys.platform == 'linux2':
            if get_os()[1:] > (2, 5, 4):
                self.assertEqual(m.engine, sockqueue._SocketQueueEPoll)
            else:
                self.assertEqual(m.engine, sockqueue._SocketQueuePoll)
                
        elif sys.platform == 'win32':
            self.assertEqual(m.engine, sockqueue._SocketQueueSelect)
            
        elif sys.platform == 'darwin' or 'bsd' in sys.platform:
            self.assertEqual(m.engine, sockqueue._SocketQueueKQueue)
    ...
    def test_epoll(self):
        m = sockqueue.SocketQueue(method=sockqueue.EPOLL)
        self.assertEqual(m.engine, sockqueue._SocketQueueEPoll)
        
    def test_select(self):
        m = sockqueue.SocketQueue(method=sockqueue.SELECT)
        self.assertEqual(m.engine, sockqueue._SocketQueueSelect)
        
    @unittest.skipIf(sys.platform == 'win32', "can't use poll on windows.")
    def test_poll(self):
        m = sockqueue.SocketQueue(method=sockqueue.POLL)
        self.assertEqual(m.engine, sockqueue._SocketQueuePoll)
...
class NotificationTestCaseGeneric(NotificationTestBase):
    ...
    def _start_sockqueue(self):
        self.sock_queue = sockqueue.SocketQueue()
...
class NotificationTestCaseEPoll(NotificationTestCaseGeneric):
    def _start_sockqueue(self):
        self.sock_queue = sockqueue.SocketQueue(method=sockqueue.EPOLL)
...
class NotificationTestCaseKQueue(NotificationTestCaseGeneric):
    def _start_sockqueue(self):
        self.sock_queue = sockqueue.SocketQueue(method=sockqueue.KQUEUE)
        

@unittest.skipIf(sys.platform == 'win32', "win32 does'nt support poll")
class NotificationTestCasePoll(NotificationTestCaseGeneric):
    def _start_sockqueue(self):
        self.sock_queue = sockqueue.SocketQueue(method=sockqueue.POLL)



class NotificationTestCaseSelect(NotificationTestCaseGeneric):
    def _start_sockqueue(self):
        self.sock_queue = sockqueue.SocketQueue(method=sockqueue.SELECT)
